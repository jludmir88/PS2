...
# Apply aforementioned function to data values
set <- set*10^(max(length(set)))
# Multiply set of data by 10^x with x being the maximum number of 0's 
# to the left of the first significant digit of any given value.
# This ensures that the first character in the following function is never 0.
sig_one <- function(n) {
...
chart_one <- hist(list_sigs, 1:10)
# Creates a histogram, with 9 bars to show the number of occurences
# of every possible significant digit.
